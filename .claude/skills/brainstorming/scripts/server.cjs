...
function maybeOpenBrowser() {
	...
	if (!process.env.BRAINSTORM_OPEN) {
		return; // opt-in: only after the user approves the companion
	}
		...
		debounceTimers.set(
			...
			setTimeout(() => {
				...
				if (knownFiles.has(filename)) {
					console.log(
						JSON.stringify({ file: filePath, type: "screen-updated" }),
					);
				} else {
